...
function voda_customize_register($wp_customize)
{
	...
	// apply
	$wp_customize->add_setting('voda_apply', array(
		'default' => '',
		'sanitize_callback' => 'esc_url',
	));

	$wp_customize->add_control('voda_apply', array(
		'label' => __('Apply Now Btn Link', 'voda'),
		'section' => 'voda_contact_section',
		'type' => 'text',
	));
}
...
function voda_hide_admin_menus_for_madiha() {
	$current_user = wp_get_current_user();

	if ($current_user->user_email === 'madihazehsadiq@gmail.com') {
			...
			remove_menu_page('edit.php?post_type=acf-field-group'); // ACF (if using Pro)
	}
}
add_action('admin_menu', 'voda_hide_admin_menus_for_madiha', 999);
